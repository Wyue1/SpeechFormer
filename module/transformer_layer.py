...
from module.utils import create_PositionalEncoding, _get_activation_fn, add_position
...
class Multihead_attention(nn.Module):
    ...
    def forward(self, query, key=None, value=None, key_padding_mask=None, attn_mask=None, need_weights=False):
        '''
        Args:
...
class TransformerEncoder(nn.Module):
    def __init__(self, self_attn, embed_dim, qdim=None, kdim=None, 
                ffn_embed_dim=2304, num_heads=8, dropout=0.1, 
                attention_dropout=0.1, activation='relu'):
        super().__init__()
        self.self_attn = self_attn
        self.dropout = dropout
    ...
    def forward(self, query, key=None, value=None, query_position=None, key_position=None, 
        ...
        """
        residual = query
        query = add_position(query, query_position)
        key = add_position(key, key_position) if not self.self_attn else key
            
        x, weights = self.attention(query, key, value, key_padding_mask, attn_mask, need_weights)
...
class TransformerBlocks(nn.Module):
    ...
    def __init__(self, self_attn=True, num_layers=1, embed_dim=1024, qdim=None, kdim=None, 
            ffn_embed_dim=2304, num_heads=8, dropout=0.1, attention_dropout=0.1, 
            activation='relu', use_position=False, **kw_args):
        super().__init__()
        self.self_attn = self_attn
        ...
        self.layers = nn.ModuleList([TransformerEncoder(self_attn, embed_dim, qdim, kdim, ffn_embed_dim, 
                                                        num_heads, dropout, attention_dropout, activation) for _ in range(num_layers)])
    ...
    def forward(self, query, key=None, key_padding_mask=None, attn_mask=None, need_weights=False):
        ...
        for layer in self.layers:
            output, weights = layer(output, key, value, self.query_position, self.key_position, key_padding_mask, attn_mask, need_weights)
...
def build_transformer(self_attn, num_layers, embed_dim, qdim=None, kdim=None, 
        ffn_embed_dim=2304, num_heads=8, dropout=0.1, attention_dropout=0.1, 
        activation='relu', use_position=False):
    '''
    ...
    return TransformerBlocks(self_attn, num_layers, embed_dim, qdim, kdim, 
                        ffn_embed_dim, num_heads, dropout, attention_dropout, 
                        activation, use_position)
